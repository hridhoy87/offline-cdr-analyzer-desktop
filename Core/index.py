...
def process_cdr_data(file_paths, intended_location, output_dir, start_ts=None, end_ts=None):
    ...
    try:
        ...
        col_A, col_C, col_D, col_E, col_F, col_H, col_I, col_J, col_L = (
            combined_df.columns[0], combined_df.columns[2], combined_df.columns[3], 
            combined_df.columns[4], combined_df.columns[5], combined_df.columns[7], 
            combined_df.columns[8], combined_df.columns[9], combined_df.columns[11]
        )
        ...
        for col in [col_F, col_H, col_I, col_L, col_J]: 
            combined_df[col] = combined_df[col].fillna('').astype(str).str.strip().replace('nan', '')
        for col in [col_H, col_I, col_J]: 
            combined_df[col] = combined_df[col].apply(lambda x: str(x).split('.')[0])
            
        combined_df = combined_df[(combined_df[col_C].str.len() == 11) & (combined_df[col_D].str.len() == 11)]
        if combined_df.empty: 
            return {"status": "error", "message": "0 valid rows remaining."}
        ...
        if total_night_count > 0:
            valid_locs = night_df[night_df[col_L] != ''][col_L].value_counts().head(5)
        ...
        # Output the compiled Excel document
        ...
        # Build structural profile telemetry maps for Javascript graph engines
        ...
        for entity_col, area_map in [(col_C, a_areas), (col_D, b_areas)]:
            for item in combined_df[entity_col].unique():
                sitem = str(item)
                if sitem and sitem != 'nan' and sitem not in node_profiles:
...
def crop_cdr_data(file_paths, location_query, start_ts, end_ts, output_dir):
    ...
    try:
        ...
        if location_query.strip(): 
            mask &= combined[combined.columns[11]].fillna('').astype(str).str.contains(location_query, case=False, na=False)
        ...
        for p in paths:
            if os.path.exists(p):
                ...
                if not df.empty and len(df.columns) >= 12:
                    ...
                    t['A'] = df[df.columns[2]].fillna('').astype(str).str.strip().apply(_clean_phone_number)
                    t['B'] = df[df.columns[3]].fillna('').astype(str).str.strip().apply(_clean_phone_number)
